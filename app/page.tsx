  ...
  const fetchProducts = useCallback(async (ids: string[], setter: Function) => {
    try {
      const responses = await Promise.all(
        ids.map((id) => axios.get(`http://localhost:3000/api/products/${id}`))
      );
      ...
      <main className="flex-1 w-2/3 justify-center">
        <section className="w-full bg-black rounded-md relative">
          ...
          <button
            ...
            className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-50 rounded-full p-2 shadow-lg hover:bg-opacity-75"
          ...
          <button
            ...
            className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-50 rounded-full p-2 shadow-lg hover:bg-opacity-75"
        ...
        <section className="w-full py-12 md:py-18 lg:py-24 bg-gray-100">
          <div className="container px-4 md:px-6">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {["Figurine", "Apparel", "Accessories", "Manga"].map((category, i) => (
                <Link href={`/catalog/filters/category/${category}`} key={i} className="relative group overflow-hidden rounded-lg shadow-lg">
                  <Image
                    alt={category}
